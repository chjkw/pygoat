...
from .models import FAANG, authLogin, comments, info, login, otp
        ...
        if name:
            ...
            if login.objects.filter(user=name):
                ...
                if val:
                    ...
                    return render(
                        request, "Lab/SQL/sql_lab.html", {"wrongpass": password}
                    )
            else:
    ...
    if request.user.is_authenticated:
        ...
        if name:
            ...
            if request.COOKIES.get("admin") == "1":
                ...
                html = render(
                    request, "Lab/BrokenAccess/ba_lab.html", {"data": "Welcome Jack"}
                )
            ...
            return render(
                request,
                "Lab/BrokenAccess/ba_lab.html",
                {"data": "Please Provide Credentials"},
            )
    else:
        return redirect("login")
...
def secret(request):
    ...
    if XHost == "admin.localhost:8000":
        return render(
            request, "Lab/sec_mis/sec_mis_lab.html", {"secret": "SECERTKEY123"}
        )
    else:
        return render(
            request,
            "Lab/sec_mis/sec_mis_lab.html",
            {"secret": "Only admin.localhost:8000 can access, Your X-Host is " + XHost},
        )
